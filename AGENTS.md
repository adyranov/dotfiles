...
- `home/.chezmoidata/base/packages.toml` is the source of truth for tools (`packages.base.*` sections keyed by toolchain, e.g. `packages.base.core`). Additional data files: `base/ai.toml` (AI agent configs, permissions) and `base/editors.toml` (VS Code/Cursor shared settings and extensions). Per-distro overrides live under `home/.chezmoidata/os/<distro>/` (aligned with `host.distro.id` and `chezmoiscripts/os/<distro>/`).
